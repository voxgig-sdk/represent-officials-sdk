...
local RepresentativeSetEntity = {}
RepresentativeSetEntity.__index = RepresentativeSetEntity


function RepresentativeSetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RepresentativeSetEntity)
  self._name = "representative_set"
  self._client = client
...
function RepresentativeSetEntity:get_name()
  return self._name
end


function RepresentativeSetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RepresentativeSetEntity.new(self._client, opts)
end
...
function RepresentativeSetEntity:mark_deleted()
  self._deleted = true
end


function RepresentativeSetEntity:deleted()
  return true == self._deleted
end


function RepresentativeSetEntity:data_set(args)
  if args ~= nil then
...
function RepresentativeSetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RepresentativeSetEntity:match_set(args)
  if args ~= nil then
...
function RepresentativeSetEntity:match_get()
...
function RepresentativeSetEntity:stream(action, args, callopts)
...
function RepresentativeSetEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return RepresentativeSetEntity
