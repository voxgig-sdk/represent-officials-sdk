...
describe("RepresentativeSetDirect", function()
  it("should direct-list-representative_set", function()
    local setup = representative_set_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-representative_set", setup.live and "live" or "unit")
  ...
  it("should direct-load-representative_set", function()
    local setup = representative_set_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-representative_set", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function representative_set_direct_setup(mockres)
