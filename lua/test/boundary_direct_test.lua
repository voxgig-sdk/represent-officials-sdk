...
describe("BoundaryDirect", function()
  it("should direct-list-boundary", function()
    local setup = boundary_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-boundary", setup.live and "live" or "unit")
  ...
  it("should direct-load-boundary", function()
    local setup = boundary_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-boundary", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function boundary_direct_setup(mockres)
