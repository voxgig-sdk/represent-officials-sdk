...
The Lua SDK for the RepresentOfficials API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Boundary(nil):list(nil, nil)
...
```lua
local result, err = client:Boundary(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:RepresentOfficials(nil):load(
  { id = "test01" }, nil
)
