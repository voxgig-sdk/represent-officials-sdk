...
require("represent-officials_types")
...
local RepresentOfficialsSDK = {}
RepresentOfficialsSDK.__index = RepresentOfficialsSDK
...
RepresentOfficialsSDK._make_feature = _make_feature


function RepresentOfficialsSDK.new(options)
  local self = setmetatable({}, RepresentOfficialsSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function RepresentOfficialsSDK:options_map()
...
function RepresentOfficialsSDK:get_utility()
  return Utility.copy(self._utility)
end


function RepresentOfficialsSDK:get_root_ctx()
  return self._rootctx
end


function RepresentOfficialsSDK:prepare(fetchargs)
...
function RepresentOfficialsSDK:direct(fetchargs)
...
function RepresentOfficialsSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function RepresentOfficialsSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "RepresentOfficialsSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function RepresentOfficialsSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function RepresentOfficialsSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "RepresentOfficialsSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function RepresentOfficialsSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = RepresentOfficialsSDK.new(sdkopts)
  sdk.mode = "test"
