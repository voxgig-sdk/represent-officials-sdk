...
(0, node_test_1.describe)('RepresentativeSetDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.RepresentOfficialsSDK({
    ...
    (0, node_test_1.test)('direct-load-representative_set', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-representative_set', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-representative_set', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-representative_set', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.REPRESENT_OFFICIALS_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    const calls = [];
    const env = (0, utility_1.envOverride)({
        'REPRESENT_OFFICIALS_TEST_REPRESENTATIVE_SET_ENTID': {},
    ...
    if (live) {
        ...
        const client = new __1.RepresentOfficialsSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['REPRESENT_OFFICIALS_TEST_REPRESENTATIVE_SET_ENTID'];
    ...
    const client = new __1.RepresentOfficialsSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=RepresentativeSetDirect.test.js.map