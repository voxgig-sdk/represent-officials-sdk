...
describe('BoundaryEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'boundary.' + op, live)) return
    }
    ...
    const boundary_ref01_list = await boundary_ref01_ent.list(boundary_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['REPRESENT_OFFICIALS_TEST_BOUNDARY_ENTID']
