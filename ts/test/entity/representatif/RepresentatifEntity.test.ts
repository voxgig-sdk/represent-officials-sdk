...
describe('RepresentatifEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'representatif.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set REPRESENT_OFFICIALS_TEST_REPRESENTATIF_ENTID JSON to run live')
      return
    }
    ...
    let representatif_ref01_data = Object.values(setup.data.existing.representatif)[0] as any

    // LIST
    const representatif_ref01_ent = client.Representatif()
    const representatif_ref01_match: any = {}
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/representatif/RepresentatifTestData.json')
  ...
  let idmap = transform(
    ['representatif01','representatif02','representatif03','boundary01','boundary02','boundary03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['REPRESENT_OFFICIALS_TEST_REPRESENTATIF_ENTID']
