...
describe('RepresentativeSetEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'representative_set.' + op, live)) return
    }
    ...
    const representative_set_ref01_list = await representative_set_ref01_ent.list(representative_set_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['REPRESENT_OFFICIALS_TEST_REPRESENTATIVE_SET_ENTID']
