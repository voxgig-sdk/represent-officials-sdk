...
//     (\`new RepresentOfficialsSDK(...)\` / \`RepresentOfficialsSDK.test(...)\` become
//     \`RepresentOfficialsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'RepresentOfficialsSDK'
...
// rewrite real client construction (\`new RepresentOfficialsSDK(...)\` and any
// \`RepresentOfficialsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RepresentOfficialsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RepresentOfficialsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
