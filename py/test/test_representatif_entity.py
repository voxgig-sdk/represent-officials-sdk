...
class TestRepresentatifEntity:

    def test_should_create_instance(self):
        testsdk = RepresentOfficialsSDK.test(None, None)
        ent = testsdk.Representatif(None)
    ...
    def test_should_stream(self):
        ...
        base = RepresentOfficialsSDK.test(seed, None)
        seen = list(base.Representatif(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = RepresentOfficialsSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Representatif(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _representatif_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "representatif." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set REPRESENT_OFFICIALS_TEST_REPRESENTATIF_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        representatif_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.representatif")))
        representatif_ref01_data = None
        if len(representatif_ref01_data_raw) > 0:
            representatif_ref01_data = helpers.to_map(representatif_ref01_data_raw[0][1])

        # LIST
        representatif_ref01_ent = client.Representatif(None)
...
def _representatif_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/representatif/RepresentatifTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "REPRESENT_OFFICIALS_TEST_REPRESENTATIF_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("REPRESENT_OFFICIALS_TEST_REPRESENTATIF_ENTID"))
