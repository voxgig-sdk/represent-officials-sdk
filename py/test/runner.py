# RepresentOfficials SDK test runner
...
class RepresentOfficialsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RepresentOfficialsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RepresentOfficialsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RepresentOfficialsTestRunner.getenv("REPRESENT_OFFICIALS_TEST_LIVE")
        override = RepresentOfficialsTestRunner.getenv("REPRESENT_OFFICIALS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RepresentOfficialsTestRunner.getenv(key)
        ...
        explain = RepresentOfficialsTestRunner.getenv("REPRESENT_OFFICIALS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["REPRESENT_OFFICIALS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if RepresentOfficialsTestRunner._test_control is not None:
            return RepresentOfficialsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RepresentOfficialsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RepresentOfficialsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RepresentOfficialsTestRunner.load_test_control()
...
def load_env_local():
    RepresentOfficialsTestRunner.load_env_local()


def env_override(m):
    return RepresentOfficialsTestRunner.env_override(m)


def entity_data(v):
    return RepresentOfficialsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RepresentOfficialsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RepresentOfficialsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RepresentOfficialsTestRunner.load_test_control()


def live_client_options():
    return RepresentOfficialsTestRunner.live_client_options()


def live_delay_ms():
    return RepresentOfficialsTestRunner.live_delay_ms()
