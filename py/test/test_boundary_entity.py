...
class TestBoundaryEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Boundary(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Boundary(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _boundary_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "boundary." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set REPRESENT_OFFICIALS_TEST_BOUNDARY_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        boundary_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.boundary")))
        boundary_ref01_data = None
        if len(boundary_ref01_data_raw) > 0:
            boundary_ref01_data = helpers.to_map(boundary_ref01_data_raw[0][1])

        # LIST
        boundary_ref01_ent = client.Boundary(None)
        boundary_ref01_match = {}

        boundary_ref01_list_result = boundary_ref01_ent.list(boundary_ref01_match, None)
...
def _boundary_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/boundary/BoundaryTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("REPRESENT_OFFICIALS_TEST_BOUNDARY_ENTID"))
