...
class TestCandidateDirect:

    def test_should_direct_list_candidate(self):
        setup = _candidate_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-candidate", "live" if setup["live"] else "unit")
        if _skip:
...
def _candidate_direct_setup(mockres):
