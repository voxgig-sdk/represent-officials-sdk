...
from representofficials_sdk.representofficials_types import (
    BoundarySet,
...
class BoundarySetEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return BoundarySetEntity(self._client, opts)
    ...
    def match_get(self) -> BoundarySet:
    ...
    def load(self, reqmatch=None, ctrl=None) -> BoundarySet:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.BoundarySet().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[BoundarySet]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.BoundarySet().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
