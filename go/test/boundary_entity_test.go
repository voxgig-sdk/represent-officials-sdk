...
func TestBoundaryEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Boundary(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Boundary(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set REPRESENT_OFFICIALS_TEST_BOUNDARY_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		boundaryRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.boundary")))
...
func boundaryBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("REPRESENT_OFFICIALS_TEST_BOUNDARY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["REPRESENT_OFFICIALS_TEST_BOUNDARY_ENTID"])
