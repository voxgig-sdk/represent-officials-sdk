...
func TestRepresentatifEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Representatif(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Representatif(nil).Stream("list", nil, nil) {
...
func representatifBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("REPRESENT_OFFICIALS_TEST_REPRESENTATIF_ENTID")
	...
	idmapResolved := core.ToMapAny(env["REPRESENT_OFFICIALS_TEST_REPRESENTATIF_ENTID"])
