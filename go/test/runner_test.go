...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("REPRESENTOFFICIALS_TEST_LIVE") == "TRUE" ||
		os.Getenv("REPRESENTOFFICIALS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("REPRESENTOFFICIALS_TEST_EXPLAIN"); explain != "" {
