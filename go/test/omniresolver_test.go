...
import (
	...
	omni "github.com/voxgig-sdk/represent-officials-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RepresentOfficialsError); is {
				out := map[string]any{
					"name":    "RepresentOfficialsError",
...
func omniCtx(arg any, client *sdk.RepresentOfficialsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RepresentOfficialsSDK); is {
			client = live
