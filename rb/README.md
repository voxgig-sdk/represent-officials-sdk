...
The Ruby SDK for the RepresentOfficials API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Boundary(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.RepresentOfficials(nil).load(
  { "id" => "test01" }, nil
)
