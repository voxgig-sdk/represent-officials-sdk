...
class CandidateDirectTest < Minitest::Test
  def test_direct_list_candidate
    setup = candidate_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-candidate", setup[:live] ? "live" : "unit")
    if _should_skip
...
def candidate_direct_setup(mockres)
