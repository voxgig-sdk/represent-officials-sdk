...
class BoundarySetEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.BoundarySet(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.BoundarySet(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set REPRESENT_OFFICIALS_TEST_BOUNDARY_SET_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    boundary_set_ref01_list_result = boundary_set_ref01_ent.list(boundary_set_ref01_match, nil)
...
def boundary_set_basic_setup(extra)
  ...
  entid_env_raw = ENV["REPRESENT_OFFICIALS_TEST_BOUNDARY_SET_ENTID"]
