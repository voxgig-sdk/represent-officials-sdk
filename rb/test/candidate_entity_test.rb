...
class CandidateEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Candidate(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Candidate(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = candidate_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "candidate." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set REPRESENT_OFFICIALS_TEST_CANDIDATE_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # Bootstrap entity data from existing test data.
    candidate_ref01_data_raw = Vs.items(Helpers.to_map(
      Vs.getpath(setup[:data], "existing.candidate")))
    candidate_ref01_data = nil
    if candidate_ref01_data_raw.length > 0
      candidate_ref01_data = Helpers.to_map(candidate_ref01_data_raw[0][1])
    end
...
def candidate_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "candidate", "CandidateTestData.json")
  ...
  entid_env_raw = ENV["REPRESENT_OFFICIALS_TEST_CANDIDATE_ENTID"]
