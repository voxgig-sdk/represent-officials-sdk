...
class RepresentOfficialsSDK
    ...
    if feature_opts
      ...
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = RepresentOfficialsHelpers.to_map(feature_opts[fname])
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => RepresentOfficialsError.new(
        "#{op}_allow",
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = RepresentOfficialsError.new(
        "graphql_error", "RepresentOfficialsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
