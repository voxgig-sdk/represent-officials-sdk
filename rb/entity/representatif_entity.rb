...
class RepresentatifEntity
  ...
  def make
    opts = @_entopts.dup
    RepresentatifEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Representatif.load works with no args).
  ...
  #
  # @param reqmatch [RepresentatifListMatch, Hash, nil] match filter (any subset of
  #   Representatif fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
