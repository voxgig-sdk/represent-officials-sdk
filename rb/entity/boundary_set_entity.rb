...
class BoundarySetEntity
  ...
  def make
    opts = @_entopts.dup
    BoundarySetEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.BoundarySet.load works with no args).
  ...
  #
  # @param reqmatch [BoundarySetListMatch, Hash, nil] match filter (any subset of
  #   BoundarySet fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
