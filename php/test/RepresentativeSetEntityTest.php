...
class RepresentativeSetEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = RepresentOfficialsSDK::test(null, null);
        $ent = $testsdk->RepresentativeSet(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->RepresentativeSet(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->RepresentativeSet(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = representative_set_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "representative_set." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set REPRESENT_OFFICIALS_TEST_REPRESENTATIVE_SET_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $representative_set_ref01_data = null;
        if (count($representative_set_ref01_data_raw) > 0) {
...
function representative_set_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/representative_set/RepresentativeSetTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["representative_set01", "representative_set02", "representative_set03"] as $k) {
    ...
    $entid_env_raw = getenv("REPRESENT_OFFICIALS_TEST_REPRESENTATIVE_SET_ENTID");
