...
class CandidateEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = RepresentOfficialsSDK::test(null, null);
        $ent = $testsdk->Candidate(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Candidate(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Candidate(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = candidate_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "candidate." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $candidate_ref01_data = null;
        if (count($candidate_ref01_data_raw) > 0) {
...
function candidate_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/candidate/CandidateTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["candidate01", "candidate02", "candidate03"] as $k) {
