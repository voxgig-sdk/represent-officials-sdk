...
// RepresentOfficials SDK pipeline test
//
...
{
    private static function utility(): RepresentOfficialsUtility
    {
        return new RepresentOfficialsUtility();
    }
    ...
    private static function ctx(array $over = []): RepresentOfficialsContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new RepresentOfficialsContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new RepresentOfficialsOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof RepresentOfficialsError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new RepresentOfficialsOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = RepresentOfficialsMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new RepresentOfficialsOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = RepresentOfficialsMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new RepresentOfficialsOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = RepresentOfficialsMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RepresentOfficialsSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = RepresentOfficialsMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new RepresentOfficialsResponse([]),
            'result' => new RepresentOfficialsResult([]),
        ]);
        [, $err] = RepresentOfficialsMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new RepresentOfficialsSpec([]),
            'response' => null,
            'result' => new RepresentOfficialsResult([]),
        ]);
        [, $err] = RepresentOfficialsMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new RepresentOfficialsSpec(['step' => 's']),
            'response' => new RepresentOfficialsResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new RepresentOfficialsResult([]),
        ]);
        [, $err] = RepresentOfficialsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new RepresentOfficialsSpec(['step' => 's']),
            'response' => new RepresentOfficialsResponse(self::resp(200, ['v' => 1])),
            'result' => new RepresentOfficialsResult([]),
        ]);
        [, $err] = RepresentOfficialsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new RepresentOfficialsSpec(['step' => 's']),
            'response' => new RepresentOfficialsResponse(self::resp(200, ['v' => 2])),
            'result' => new RepresentOfficialsResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        RepresentOfficialsMakeResponse::call($ctx);
    ...
    {
        $preset = new RepresentOfficialsResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new RepresentOfficialsSpec([]),
            'response' => new RepresentOfficialsResponse([]),
            'result' => new RepresentOfficialsResult([]),
        ...
        [$got, $err] = RepresentOfficialsMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new RepresentOfficialsResult([])]);
        [, $err] = RepresentOfficialsMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new RepresentOfficialsSpec([]), 'result' => null]);
        [, $err] = RepresentOfficialsMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RepresentOfficialsSpec(['step' => 's']),
            'result' => new RepresentOfficialsResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new RepresentOfficialsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RepresentOfficialsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RepresentOfficialsSpec(['step' => 's']),
            'result' => new RepresentOfficialsResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new RepresentOfficialsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RepresentOfficialsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RepresentOfficialsResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new RepresentOfficialsSpec([]),
            'result' => new RepresentOfficialsResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = RepresentOfficialsMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RepresentOfficialsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new RepresentOfficialsError('boom', 'boom');
        $utility->fetcher = function (RepresentOfficialsContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RepresentOfficialsSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = RepresentOfficialsMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (RepresentOfficialsContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = RepresentOfficialsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(RepresentOfficialsResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        RepresentOfficialsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (RepresentOfficialsContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RepresentOfficialsSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = RepresentOfficialsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new RepresentOfficialsResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new RepresentOfficialsSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = RepresentOfficialsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = RepresentOfficialsMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = RepresentOfficialsMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new RepresentOfficialsResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, RepresentOfficialsDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new RepresentOfficialsResult(['ok' => false])]);
        $this->expectException(RepresentOfficialsError::class);
        RepresentOfficialsDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new RepresentOfficialsResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', RepresentOfficialsMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        RepresentOfficialsMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new RepresentOfficialsBaseFeature();
        $b = new RepresentOfficialsBaseFeature();
        RepresentOfficialsFeatureAdd::call($ctx, $a);
        RepresentOfficialsFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): RepresentOfficialsBaseFeature
    {
        $f = new RepresentOfficialsBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        RepresentOfficialsFeatureAdd::call($ctx, self::named_feature('a'));
        RepresentOfficialsFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        RepresentOfficialsFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        RepresentOfficialsFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        RepresentOfficialsFeatureAdd::call($ctx, $replace);
        ...
        RepresentOfficialsFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): RepresentOfficialsContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new RepresentOfficialsSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RepresentOfficialsPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        RepresentOfficialsPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new RepresentOfficialsResponse(['headers' => null]),
            'result' => new RepresentOfficialsResult([]),
        ]);
        RepresentOfficialsResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        RepresentOfficialsResultBody::call($ctx);
