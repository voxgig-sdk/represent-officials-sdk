...
// RepresentOfficials SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = RepresentOfficialsOmni::makeStructRunner(
    ...
    {
        return RepresentOfficialsOmni::objmodel(RepresentOfficialsOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return RepresentOfficialsOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return RepresentOfficialsOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(RepresentOfficialsOmni::structfix($now, Struct::undef()))
                        !== json_encode(RepresentOfficialsOmni::structfix($val, Struct::undef()))) {
