...
class BoundarySetEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = RepresentOfficialsSDK::test(null, null);
        $ent = $testsdk->BoundarySet(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->BoundarySet(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->BoundarySet(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = boundary_set_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "boundary_set." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set REPRESENT_OFFICIALS_TEST_BOUNDARY_SET_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $boundary_set_ref01_data = null;
        if (count($boundary_set_ref01_data_raw) > 0) {
...
function boundary_set_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/boundary_set/BoundarySetTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["boundary_set01", "boundary_set02", "boundary_set03"] as $k) {
    ...
    $entid_env_raw = getenv("REPRESENT_OFFICIALS_TEST_BOUNDARY_SET_ENTID");
