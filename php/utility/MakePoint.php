...
// RepresentOfficials SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class RepresentOfficialsMakePoint
{
    public static function call(RepresentOfficialsContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof RepresentOfficialsError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = RepresentOfficialsHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = RepresentOfficialsHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
