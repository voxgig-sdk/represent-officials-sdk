...
type Args struct {
	Entity string         `json:"entity" jsonschema:"boundary | boundary_set | candidate | election | postal_code | representatif | representative_set"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.RepresentOfficialsSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "represent-officials_list",
		Description: "List records from RepresentOfficials. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "represent-officials_load",
		Description: "Load a single record from RepresentOfficials. " +
...
func runOp(client *sdk.RepresentOfficialsSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.RepresentOfficialsSDK, name string) (sdk.RepresentOfficialsEntity, error) {
	switch strings.ToLower(name) {
	case "boundary":
		return client.Boundary(nil), nil
	case "boundary_set":
		return client.BoundarySet(nil), nil
	case "candidate":
		return client.Candidate(nil), nil
	case "election":
