...
MCP server exposing the RepresentOfficials SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `represent-officials_list` | `entity`, optional `query` map | First page of records as JSON |
| `represent-officials_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
```sh
go build -o represent-officials-mcp ./...
```
...
After install, restart Claude Code; the `represent-officials_list` and
`represent-officials_load` tools become available in new sessions.
