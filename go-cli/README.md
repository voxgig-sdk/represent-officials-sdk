# represent-officials-cli

boru-driven command-line client **and** interactive REPL for the RepresentOfficials
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/represent-officials-cli)
make build

# 2. See usage (words, entities, env vars)
./represent-officials-cli --help

# 3. Provide credentials once, via the environment
export REPRESENT_OFFICIALS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./represent-officials-cli list boundary
./represent-officials-cli load 1 boundary            # {id:1} shorthand
./represent-officials-cli load '{id:1}' boundary       # explicit match map
./represent-officials-cli list boundary_set

# 5. Override the API base URL for a single call
REPRESENT_OFFICIALS_BASE=https://api.example.com ./represent-officials-cli list boundary

# 6. No arguments -> interactive REPL
./represent-officials-cli
represent-officials> list boundary
represent-officials> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/represent-officials-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/represent-officials-cli list boundary
   ```
...
```sh
./represent-officials-cli load 1 boundary          # scalar shorthand for {id:1}
./represent-officials-cli load '{id:1}' boundary     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `represent-officials>`). Each line is
evaluated as its own boru expression:

```text
$ ./represent-officials-cli
represent-officials> list boundary
represent-officials> /help
represent-officials> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `boundary`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/represent-officials-cli`. |
...
(`./represent-officials-cli <expr>`) and interactively (the REPL), and expressions compose the
