...
module RepresentOfficialsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RepresentOfficialsResponse.new({})
    ...
    if fetch_err
      ...
      response = RepresentOfficialsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RepresentOfficialsResponse.new(fetched)
    else
