...
module RepresentOfficialsUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = RepresentOfficialsContext.new({}, nil)
    end
    op = ctx.op || RepresentOfficialsOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || RepresentOfficialsResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(RepresentOfficialsError) ? err.msg : err.to_s
    ...
    sdk_err = RepresentOfficialsError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(RepresentOfficialsError)
