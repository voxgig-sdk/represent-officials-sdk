...
class RepresentOfficialsMakeError
{
    public static function call(?RepresentOfficialsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new RepresentOfficialsContext([], null);
        }
        $op = $ctx->op ?? new RepresentOfficialsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new RepresentOfficialsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof RepresentOfficialsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new RepresentOfficialsError('', $msg, $ctx);
        ...
        if ($err instanceof RepresentOfficialsError) {
