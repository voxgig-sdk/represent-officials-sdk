...
// RepresentOfficials SDK retry feature
...
class RepresentOfficialsRetryFeature extends RepresentOfficialsBaseFeature
{
    ...
    public function init(RepresentOfficialsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RepresentOfficialsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RepresentOfficialsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RepresentOfficialsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
