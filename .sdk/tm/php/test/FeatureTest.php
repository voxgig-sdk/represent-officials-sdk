...
// RepresentOfficials SDK feature test
//
...
class FtCtrl extends RepresentOfficialsControl
{
...
class FtClient
{
    ...
    private RepresentOfficialsUtility $utility;

    public function __construct(RepresentOfficialsUtility $utility, array $options)
    {
    ...
    public function get_utility(): RepresentOfficialsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RepresentOfficialsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RepresentOfficialsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RepresentOfficialsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RepresentOfficialsConfig::make_feature($name);
    ...
    private static function buildUrl(RepresentOfficialsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RepresentOfficialsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RepresentOfficialsResponse($fetched);
        }
    ...
    private function fail(RepresentOfficialsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RepresentOfficialsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
